...
void setup() {
    ...
    while (year() <= 2000) {
      ...
      if (WiFi.status() != WL_CONNECTED){
        WiFi.reconnect();
        if(counter++ > 1000 && !WiFi.isConnected()) ESP.reset();  // Restart the ESP after x attempts
    ...
    if (EEPROM.read(109) != 1){               // Check if colours have been set or not
...
void loop() {
    ...
    if (WiFi.status() != WL_CONNECTED){
      WiFi.reconnect();
      if(counter++ > 5000 && !WiFi.isConnected()) ESP.reset();  // Restart the ESP after 150 attempts
      else if(WiFi.isConnected()) counter = 0;                // Reset counter if connected
    }
...
void showTime(int hr, int mn, int sec) {
  ...
  //fill_solid(hourleds, 12, CRGB::FireBrick);
  if(hr == 24)
    hourleds[0]=hours;
  if(hr>=12) 
    hourleds[23-hr]=hours;
  if(hr <12)
    hourleds[11-hr]=hours;
  ...
  //reverseLeds();
  if(hr < 6 || hr >= 23)
    FastLED.setBrightness(0); // Set brightness to light_low during night - cools down LEDs and power supplies.
  else
    FastLED.setBrightness(constrain(light_high,10,255));
  //
}
...
void effects(){
  for( int j = 0; j< 300; j++){
    fadeToBlackBy( leds(NUM_LEDS,0), NUM_LEDS, 20);
    byte dothue = 0;
    for( int i = 0; i < 8; i++) {
      leds(NUM_LEDS,0)[beatsin16(i+7,0,NUM_LEDS)] |= CHSV(dothue, 200, 255);
      dothue += 32;
    }
    //reverseLEDs();
    FastLED.show();
  ...
  fill_solid(leds, NUM_LEDS +12, bg);
  //client.publish("smallinfinity/status","RAINBOW");
  lastsec = 1;
}
...
time_t getNtpTime() {
  ...
  while (millis() - beginWait < 1600) {
    ...
    if (size >= NTP_PACKET_SIZE) {
      ...
      // client.publish("smallinfinity/status",secsSince1900 - 2208988800UL + timeZone * SECS_PER_HOUR);
      return secsSince1900 - 2208988800UL + timeZone * SECS_PER_HOUR;

    }
  }
  //client.publish("smallinfinity/status","NTP Time Updated");
